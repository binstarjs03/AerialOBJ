...
{
    public Point2<float> ConvertWorldToScreen(Point2Z<float> worldPos, Point2Z<float> cameraPos, float unitMultiplier, Size<int> screenSize)
    {
        Point2<float> unitOffsetScaled = (Point2<float>)(-(cameraPos * unitMultiplier));
        Point2<float> screenCenter = new (screenSize.Width / 2,
                                          screenSize.Height / 2);

        Point2<float> scaledPoint = (Point2<float>)(worldPos * unitMultiplier);
